...
export const ProgressiveBlurView: FC<Props> = ({ position = 'top', height = 100, blurViewProps }) => {
  ...
  return (
    <View
      className={cn('absolute left-0 right-0 pointer-events-none', position === 'top' ? 'top-0' : 'bottom-0')}
      style={[
        {
          position: 'absolute',
          height,
    ...
    >
      {Platform.OS === 'ios' ? (
        ...
        <LinearGradient
          ...
          colors={
            position === 'top'
              ? [colorKit.setAlpha(themeColorBackground, 0.9).hex(), colorKit.setAlpha(themeColorBackground, 0).hex()]
              : [colorKit.setAlpha(themeColorBackground, 0).hex(), colorKit.setAlpha(themeColorBackground, 0.9).hex()]
          }
        />
      )}
    </View>
